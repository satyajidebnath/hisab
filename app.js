...
function getSupabaseLibrary(){
  ...
  try{ if(typeof supabase!=='undefined' && supabase && typeof supabase.createClient==='function'){ if(typeof window!=='undefined' && !window.supabase) window.supabase=supabase; return supabase; } }catch(_){ }
  return null;
}
function createSBClient(url,key){
  const supabaseLib=getSupabaseLibrary();
  if(!supabaseLib) throw new Error("Sync library did not load. Please refresh the page.");
...
function supabaseIsConfigured(){
  ...
  return !!(c.url && c.key && getSupabaseLibrary());
}
function getSB(){
  if(sbClient) return sbClient;
  const c=getSupabaseConfig();
  if(!c.url || !c.key) throw new Error("Sync is not configured");
  if(!getSupabaseLibrary()) throw new Error("Sync library did not load. Please refresh the page.");
...
if(typeof window!=='undefined'){
  window.supabaseIsConfigured=supabaseIsConfigured;
  window.supabaseConfigure=supabaseConfigure;
  ...
  window.supabaseListProfiles=supabaseListProfiles;
  window.supabaseGetProfile=supabaseGetProfile;
  window.supabaseCreateStaffAccount=supabaseCreateStaffAccount;
  window.supabaseSetPermissions=supabaseSetPermissions;
  window.supabaseDeactivateUser=supabaseDeactivateUser;
  window.supabasePullState=supabasePullState;
  window.supabasePushState=supabasePushState;
  window.supabaseStartRealtime=supabaseStartRealtime;
  window.supabaseOnAuthStateChange=supabaseOnAuthStateChange;
  window.supabaseResolveAttachment=supabaseResolveAttachment;
}
...
async function ensureSyncScriptLoaded(){
  if(typeof supabasePushState==='function' && typeof supabaseIsConfigured==='function') return;
  throw new Error('Sync client is missing from app.js. Please upload the latest app.js file.');
}
/* ============================================================
...
async function saveDB(options={}){
  ...
  try{
    ...
    await ensureSyncScriptLoaded();
    if(typeof supabaseIsConfigured!=="function") throw new Error("Sync checker did not load. Please refresh the page.");
    if(!supabaseIsConfigured()){
      const loaded=!!window.__hisabSupabaseClientLoaded;
      const hasLib=!!(window.supabase && typeof window.supabase.createClient==="function");
      const hasConfig=!!(window.BAHIKHATA_SUPABASE && window.BAHIKHATA_SUPABASE.url && window.BAHIKHATA_SUPABASE.key);
      throw new Error("Sync is not configured. Details: script="+loaded+", library="+hasLib+", config="+hasConfig);
    }
    ensureEntryIds();
...
async function initAuth(){
  try{ await ensureSyncScriptLoaded(); }
  catch(e){ console.error(e); showLogin(e.message||"Could not load sync"); return false; }
  if(typeof supabaseIsConfigured!=="function" || !supabaseIsConfigured()){
    showLogin("Sync is not configured");
    return false;
